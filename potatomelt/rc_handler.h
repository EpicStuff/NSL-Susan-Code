...
// Compute a checksum for the current position of the sticks, to use in the
// previous
...
// returns RC_FORBACK_FORWARD, RC_FORBACK_NEUTRAL or RC_FORBACK_BACKWARD
...
// Returns (1, 16) for translation trim - adjusts how hard the bot tries to
// translate
...
#define RC_CHANNEL_TURN \
  0  // Logical turning - usually left-right on the right stick
#define RC_CHANNEL_FORBACK \
  1  // Translate forwards or backwards - usually forwards-backwards on the
     // right stick
#define RC_CHANNEL_THROTTLE \
  2  // Spin RPM - usually forwards-backwards on the left stick
...
#define RC_CHANNEL_SPIN_DIR \
  6  // Spin direction switch - runs motors forwards when >0, backwards when <0.
     // Does not adjust heading LED offsets - useful when ESCs are wired
     // backwards
#define RC_CHANNEL_TRANSLATION_TRIM \
  7  // Translate power trim - Usually a knob, more trim = robot will try harder
     // to translate while spinning.

#define DEFAULT_TRANSLATION_TRIM \
  2.0;  // The default translation trim, if the feature is disabled in
        // melty_config.h

// All pulse lengths in microseconds
// it's accepted that a TX with fully centered trims may produce values somewhat
// off these numbers

// RC pulses outside this range are considered invalid (indicate a bad RC
// signal)
#define MAX_RC_PULSE_LENGTH 2012
#define MIN_RC_PULSE_LENGTH 988

// This value reflects nominal range of possible RC pulse values (maximum -
// minimum) This value is used to help scale left / right adjustment of heading
