...
void setup() {
  ...
  init_rc();
  init_accel();  // accelerometer uses i2c - which can fail blocking (so only
                 // initializing it -after- the watchdog is running)
...
// if JUST_DO_DIAGNOSTIC_LOOP - then we just loop and display debug info via USB
// (good for testing)
  ...
  while (1) {
    ...
    service_watchdog();
    echo_diagnostics();
    delay(250);  // delay prevents serial from getting flooded (can cause issues
                 // programming)
  }
#endif
  ...
  wait_for_rc_good_and_zero_throttle();  // Wait for good RC signal at zero
                                         // throttle
  delay(MAX_MS_BETWEEN_RC_UPDATES +
        1);  // Wait for first RC signal to have expired
  wait_for_rc_good_and_zero_throttle();  // Verify RC signal is still good /
                                         // zero throttle
#endif
}
...
static void echo_diagnostics() {
  ...
  Serial.print(rc_get_throttle_perk());
  Serial.print("  RC L/R: ");
  Serial.print(rc_get_leftright());
  Serial.print("  RC F/B: ");
  Serial.print(rc_get_forback_trans());
...
static void display_rpm_if_requested() {
  ...
  if (rc_get_forback_bit() == RC_FORBACK_FORWARD) {
    ...
    if (rc_get_forback_bit() == RC_FORBACK_FORWARD &&
        rc_get_throttle_perk() == 0) {
      // throttle up cancels RPM count
      for (int x = 0; x < get_max_rpm() && rc_get_throttle_perk() == 0;
           x = x + 100) {
        service_watchdog();  // flashing out RPM can take a while - need to
                             // assure watchdog doesn't trigger
        delay(600);
        heading_led_on(0);
        delay(20);
        heading_led_off();
      }
      delay(1500);  // flash-out punctuated with delay to make clear RPM count
                    // has completed
    }
  }
}
...
static void check_config_mode() {
  // if user pulls control stick back for 750ms - enters (or exits) interactive
  // configuration mode
