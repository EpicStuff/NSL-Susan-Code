...
// returns RC_FORBACK_FORWARD, RC_FORBACK_BACKWARD or RC_FORBACK_NEUTRAL based
// on stick position
...
// returns offset in microseconds from center value (not converted to
// percentage) 0 for hypothetical perfect center (reality is probably +/-50)
...
bool rc_get_accel_save() {
  return IBus.readChannel(RC_CHANNEL_ACCEL_OFFSET_SAVE) >
         CENTER_FORBACK_PULSE_LENGTH;
}

int rc_get_spin_dir() {
  return (IBus.readChannel(RC_CHANNEL_SPIN_DIR) > CENTER_FORBACK_PULSE_LENGTH)
             ? 1
             : -1;
}
...
float rc_get_trans_trim() {
  ...
  int trim_setting =
      IBus.readChannel(RC_CHANNEL_TRANSLATION_TRIM) - MIN_RC_PULSE_LENGTH;
...
// If the checksum hasn't changed at all in too long, the connection has gone
// stale.
